...
with DAG(
    dag_id="3_2_lcom_dw_manual_run_training_sessions",
    description="LCom DW: dbt Manual run - training sessions (manual trigger only)",
    ...
    notify_summary = create_notify_summary_task(
        dag,
        run_name="LCom DW: dbt Manual run - training session",
    )
