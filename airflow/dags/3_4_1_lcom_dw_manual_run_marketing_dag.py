...
with DAG(
    dag_id="3_4_lcom_dw_manual_run_marketing",
    description="LCom DW: dbt Manual run - marketing (manual trigger only)",
    default_args=default_args,
    start_date=datetime(2024, 1, 1),
    schedule=None,          
    max_active_runs=1,      
    catchup=False,
    tags=["dbt", "lcom_dw", "marketing", "maintenance" ],
) as dag:
    ...
    core_task = make_run_lcom_dw_marketing_task(dag,
        run_type="Manual Prod run - tag:marketing",
    )

    # Shared summary email at the end
    notify_summary = create_notify_summary_task(
        dag,
        run_name="LCom DW: dbt Manual run - marketing",
    )
