...
from colibri_lineage_test import get_column_lineage_test
...
def run_schema_drift_analysis():
    ...
    missing_columns_lineage_path = get_compiled_sql_path("missing_columns_lineage_test.sql")
...
def to_html_table(rows: List[Dict[str, Any]]) -> str:
    headers = ["source", "source_column", "direct_usage", "downstream_usage", "error", "Reason"]
...
def generate_html_report(profiles_results, missing_columns_results):
    ...
    if missing_columns_results:
        ...
        for row in missing_columns_results:
            ...
            lineage_info = get_column_lineage_test(MANIFEST_PATH, source_name, column_name, flg)
            outputs.append(lineage_info)
            if lineage_info["error"]!="Paths found, but none reached any model.* nodes.":
                logging.info(f"{source_name}.{column_name} is used in models. Base profile need to be reviewed")
...
with DAG(
    dag_id="sfdc_schema_drift_audit_test",
    ...
    compile_query = BashOperator(
            ...
            bash_command=(
                ...
                "dbt compile --select missing_columns_lineage_test profiles_stats"
            ),
    ...
    # Final wiring
    # -----------------------------
    compile_query >> run_analysis >> notify_summary
