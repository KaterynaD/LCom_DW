...
with DAG(
    dag_id="3_4_lcom_dw_manual_run_revenue",
    ...
    core_task = make_run_lcom_dw_revenue_task(dag,
        run_type="Manual Prod run - tag:revenue",
    )

    # Shared summary email at the end
    notify_summary = create_notify_summary_task(
        dag,
        run_name="LCom DW: dbt Manual run - revenue",
    )
