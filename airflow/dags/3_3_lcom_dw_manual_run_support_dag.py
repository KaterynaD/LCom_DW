...
with DAG(
    dag_id="3_3_lcom_dw_manual_run_support",
    description="LCom DW: dbt Manual run - support (manual trigger only)",
    default_args=default_args,
    start_date=datetime(2024, 1, 1),
    schedule_interval=None,
    catchup=False,
    tags=["dbt", "lcom_dw", "support", "maintenance" ],
) as dag:
    ...
    core_task = BashOperator(
        task_id="run_lcom_dw_manual_run_support",
        bash_command=(
            f"cd {DBT_LCOM_DW_PROJECT_DIR} && "
            "dbt run "
            "--select tag:support "
            "--exclude \"config.materialized:view\" "
            "--vars '{\"run_type\": \"Manual Prod run - support\"}'"
        ),
    ...
    notify_summary = create_notify_summary_task(
        dag,
        run_name="LCom DW: dbt Manual run - support",
    )
