...
CREATE TABLE content_delivery_usage.fact_students_usage_monthly_snapshots
(
...
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.schoolyear_mon IS 'School Year Month number starting from August - 1, ending July - 12.';
...
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.organization_school_id IS 'The lowest level in this table: School, for school_cnt_students and school_students_launches.';
...
--
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.school_cnt_students IS 'Unique Count of Students launched an assignment at least once per school from the first day of a School Year to the last day of a month including.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.school_students_launches IS 'Unique Count of assignments launches from students per school from the first day of a School Year to the last day of a month including..';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.district_cnt_students IS 'Unique Count of Students launched an assignment at least once per district from the first day of a School Year to the last day of a month including.. The same district-level number is repeated for each School.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.district_students_launches IS 'Unique Count of assignments launches from students per district from the first day of a School Year to the last day of a month including.. The same district-level number is repeated for each School.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.state_cnt_students IS 'Unique Count of Students launched an assignment at least once per state from the first day of a School Year to the last day of a month including.. The same state-level number is repeated for each School and District.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.state_students_launches IS 'Unique Count of assignments launches from students per state from the first day of a School Year to the last day of a month including.. The same state-level number is repeated for each School and District.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.country_cnt_students IS 'Unique Count of Students launched an assignment at least once per country from the first day of a School Year to the last day of a month including.. The same country-level number is repeated for each State, School, and District.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.country_students_launches IS 'Unique Count of assignments launches from students per country from the first day of a School Year to the last day of a month including.. The same country-level number is repeated for each State, School, and District.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.company_cnt_students IS 'Total Unique Count of Students launched an assignment at least once from the first day of a School Year to the last day of a month including.. The same Company (LCOM)-level number is repeated for each Country, State, School, and District.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.company_students_launches IS 'Total Unique Count of assignments launches from students from the first day of a School Year to the last day of a month including.. The same Company (LCOM)-level number is repeated for each Country, State, School, and District.';
--
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.school_cnt_students_month IS 'Unique Count of Students launched an assignment at least once per school from the first to the last day of a month including.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.school_students_launches_month IS 'Unique Count of assignments launches from students per school.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.district_cnt_students_month IS 'Unique Count of Students launched an assignment at least once per district from the first to the last day of a month including. The same district-level number is repeated for each School.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.district_students_launches_month IS 'Unique Count of assignments launches from students per district from the first to the last day of a month including. The same district-level number is repeated for each School.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.state_cnt_students_month IS 'Unique Count of Students launched an assignment at least once per state from the first to the last day of a month including. The same state-level number is repeated for each School and District.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.state_students_launches_month IS 'Unique Count of assignments launches from students per state from the first to the last day of a month including. The same state-level number is repeated for each School and District.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.country_cnt_students_month IS 'Unique Count of Students launched an assignment at least once per country from the first to the last day of a month including. The same country-level number is repeated for each State, School, and District.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.country_students_launches_month IS 'Unique Count of assignments launches from students per country from the first to the last day of a month including. The same country-level number is repeated for each State, School, and District.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.company_cnt_students_month IS 'Total Unique Count of Students launched an assignment at least once from the first to the last day of a month including. The same Company (LCOM)-level number is repeated for each Country, State, School, and District.';
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.company_students_launches_month IS 'Total Unique Count of assignments launches from students from the first to the last day of a month including. The same Company (LCOM)-level number is repeated for each Country, State, School, and District.';
--
COMMENT ON COLUMN content_delivery_usage.fact_students_usage_monthly_snapshots.loaddate IS 'Timestamp when the record was created.';
